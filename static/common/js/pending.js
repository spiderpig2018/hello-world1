...
$.ajax({
  url: AllUrl + "/v2/bill/sales/list/number",
...
function page(data) {
  ...
  $.jqPaginator('#pagination2', {
    ...
    onPageChange: function (num, ) {
      $.ajax({
        url: AllUrl + "/v2/bill/sales/list/" + num,
        async: false,//同步方式发送请求，true为异步发送
        type: "GET",
        headers: {
          'authorization': Token
        ...
        success: function (data) {
          ...
          if (dataList.length > 0) {
            dataList.forEach(function (ele, index) {
              ...
              localCompany("S_");
              tr += `<tr><td>${ele.id}</td>`
              tr += `<td>${ele.bill_name}</td>`
              tr += `<td>${time}&nbsp<span style="font-size:12px"></span></td>`
              tr += `<td>${Acc[ele.account]}</td>`
              tr += `<td>${ele.receiva}</td>`
              tr += `<td>${ele.pay}</td>`
              tr += `<td>${Config.BillClass[ele.type]}</td>`
              tr += `<td><span class = "susasd" class="btn btn-hover btn-dark btn-block" data-toggle="modal" data-target="#myModal">${Config.billingState[ele.status]}</span></td>`
              // tr += `<td>${Acc[ele.account]}</td></tr>`
